...
"      -i, --insert-size                best tip/island cut size (default: N50+1000) \n"
...
void parseAsmLongOptions(int argc, char** argv)
{
	...
	if(asmlongopt::insertSize == 0)
	{
		std::string filename = stripFilename( asmlongopt::asqgFile )+".fa";
		asmlongopt::insertSize = estimateSizeCutoff(filename);
	}
...
void sequentialTrimAndSmooth (StringGraph* pGraph, size_t trimLength, bool bIsGapPrecent)
{
	...
	SGSmoothingVisitor smoothingVisit(asmlongopt::maxIndelLength, bIsGapPrecent);
...
size_t estimateSizeCutoff(std::string filename)
{
	std::cerr << "\nEstimating best tip/island size cutoff in " << filename << ": ";
	...
	while(reader.get(record))
		contigLengths.push_back(record.seq.length());
	...
	size_t sizeCutoff = n50+1000;
	std::cout << sizeCutoff << std::endl;
